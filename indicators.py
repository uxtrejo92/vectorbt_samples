...
def bullish_engulfing(data: pd.DataFrame, detection: str = None):
    ...
    if detection == "SMA50":
        ...
        down_trend = np.where(
            (data.close.values < sma50) & (
                data.close.values < sma200), True, False
        )
    else:
    ...
    body_high = np.maximum(data.close.values, data.close.values)
    body_low = np.minimum(data.close.values, data.close.values)
    ...
    white_body = data.close.values < data.close.values
    black_body = data.close.values > data.close.values
...
def find_swings(data: pd.DataFrame, n: int = 2) -> pd.DataFrame:
    ...
    signed_grad = np.sign(
        (swing_data - swing_data.shift(1)).fillna(method="bfill"))
    swings = (signed_grad != signed_grad.shift(
        1).fillna(method="bfill")) * -signed_grad
    ...
    if method == 0:
        ...
        for i in range(len(classified_price_swings)):
            ...
            price_at_indi_lows_change = np.sign(price_at_indi_lows) * (
                price_at_indi_lows
                - price_at_indi_lows.replace(to_replace=0,
                                             method="ffill").shift()
            )
...
def heikin_ashi(data: pd.DataFrame):
    ...
    ha_close = 0.25 * (
        working_data.close + working_data.low + working_data.high + working_data.close
    )
    ha_open = 0.5 * (working_data.close + working_data.close)
    ha_high = np.maximum(
        working_data.high.values, working_data.close.values, working_data.close.values
    )
    ha_low = np.minimum(
        working_data.low.values, working_data.close.values, working_data.close.values
    )

    ha_data = pd.DataFrame(
        data={"close": ha_open, "high": ha_high,
...
def ha_candle_run(ha_data: pd.DataFrame):
    ...
    heikin_ashi
    """
    green_candle = np.where(ha_data.close - ha_data.close > 0, 1, 0)
    red_candle = np.where(ha_data.close - ha_data.close < 0, 1, 0)
...
def cross_values(
    ...
    if isinstance(ts1, pd.Series):
        # Convert to Series
        cross_points = pd.Series(
            data=cross_points, index=ts1.index, name="crossval")
...
def build_multiplier_grid(
    ...
    pos_levels = [round(origin + direction * spacing * i, precision)
                  for i in levels]
...
def create_bricks(data: pd.DataFrame, brick_size: float = 0.002, column: str = "close"):
    ...
    bricks = pd.DataFrame(
        data={"close": opens, "close": opens}, index=close_times)
    ...
    if rng_type == 1:
        ...
        for i in range(1, len(h)):
            if h[i] >= r_filt[i - 1] + r[i]:
                ...
                r_filt[i] = (
                    r_filt[i - 1] -
                    np.floor(abs(l[i] - r_filt[i - 1]) / r[i]) * r[i]
                )
            else:
                r_filt[i] = r_filt[i - 1]
